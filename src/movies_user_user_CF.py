...
def import_data(matrix_type="partial"):
    ...
    elif matrix_type == "preprocess_item":
        path = os.path.join(DIR, '../data/FINAL_PREPROCESSING.csv')
    elif matrix_type == "user_item":
        path = os.path.join(DIR, '../data/prova_csv_z.csv')
    elif matrix_type == "user_item_weighted":
        path = os.path.join(DIR, '../data/FINAL_WEIGHTED_RAW.csv')
...
def preprocess_query(partial_utility_matrix_df,relational_data_df):
    ...
    for item in items:
        ...
        print("preprocessing "+str(item_count))
        item_count+=1
    csv_path= os.path.join(DIR, '../data/FINAL_PREPROCESSING.csv')
    pd.DataFrame(data=output_dictionary, index=queries).to_csv(csv_path)            
...
def create_relational_data_utility_matrix(partial_utility_matrix_df,preprocess_data_df):
    ...
    csv_path= os.path.join(DIR, '../data/prova_csv_p.csv')
...
def create_relational_data_utility_matrix_weighted(partial_utility_matrix_df,preprocess_data_df):
    ...
    csv_path= os.path.join(DIR, '../data/FINAL_WEIGHTED_RAW.csv')
    relational_data_utility_matrix.to_csv(csv_path)
...
def preprocess_data(utility_matrix, n_items=100, n_users=100):
    ...
    utility_matrix = utility_matrix.iloc[:n_users, :n_items]
    ...
    for user in range(utility_matrix.shape[0]):
        if utility_matrix.iloc[user, :].mean() == 0:
            for query in range(utility_matrix.shape[1]):
                if utility_matrix.iloc[user, query] == 0:  # 0s, not NAs

                    centered_matrix.iloc[user, query] = round((utility_matrix.iloc[:, query].mean()) / 10)
...
def calculate_rating(utility_matrix, centered_matrix, similarities, counter, user=4, item=0, top_n=2, verbose=False):
    ...
    for i in top_n_similarities:
        ...
        if np.isnan(utility_matrix.iloc[i, item]):
            rating += similarities[i] * utility_matrix.iloc[i, :].mean()
        else:
            rating += similarities[i] * utility_matrix.iloc[i, item]
    ...
    if similarities[top_n_similarities].sum() == 0:
        ...
        # option 1: return the mean of the user
        rating = utility_matrix.iloc[user, :].mean()

        # option 2: return the mean of the item
    ...
    for row in range(utility_matrix.shape[0]):
        for col in range(utility_matrix.shape[1]):
            if np.isnan(utility_matrix.iloc[row, col]):
                ...
                rating, counter_exception = calculate_rating(utility_matrix, centered_matrix, similarities,counter, user=row, item=col, top_n=top_n, verbose=verbose)
        ...
        print('User: ', row, 'Estimated percentage of completion: ', round((row / utility_matrix.shape[0]) * 100, 2), '%')
...
def compute_difference(utility_matrix_complete, n_items=100, n_users=100):
    ...
    real_utility_matrix_complete = real_utility_matrix_complete.iloc[:n_users, :n_items]
...
def print_data_to_html(utility_matrix_before_pp, partial_utility_matrix, centered_matrix, utility_matrix_complete, difference_utility_matrix):
    with open(os.path.join(DIR, '../data/viz.html'), 'w') as f:
...
def prepare_matrix(matrix, n_items=100, n_users=100):
    matrix = matrix.iloc[:n_users, :n_items]
    return matrix
...
if __name__ == '__main__':
    ...
    #create_relational_data_utility_matrix(partial_utility_matrix_df,preprocess_data_df)

    create_relational_data_utility_matrix_weighted(partial_utility_matrix_df,preprocess_data_df)

    start_time_load = time.time()
    partial_utility_matrix = import_data('user_item_weighted')
    end_time_load = time.time()
    ...
    proposed_utility_matrix=create_utility_matrix_from_item_matrix(items_utility_matrix_complete,preprocess_data_df,partial_utility_matrix_df)
    ...
    print_data_to_html(utility_matrix_before_pp, partial_utility_matrix, centered_matrix, proposed_utility_matrix.T, difference_utility_matrix)
