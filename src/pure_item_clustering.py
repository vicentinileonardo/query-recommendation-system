...
def import_data(matrix_type="partial"):
    if matrix_type == "partial":
        ...
        path = os.path.join(DIR, '../data/pure_item_clustering/preprocessed_queries.csv')
    elif matrix_type == "user_item_weighted":
        path = os.path.join(DIR, '../data/pure_item_clustering/partial_users_items_utility_matrix.csv')
...
def preprocess_query(partial_utility_matrix_df,relational_data_df):
    ...
    csv_path= os.path.join(DIR, '../data/pure_item_clustering/preprocessed_queries.csv')
...
def create_relational_data_utility_matrix(partial_utility_matrix_df,preprocess_data_df):
    ...
    csv_path= os.path.join(DIR, '../data/pure_item_clustering/prova_csv_p.csv')
...
def create_relational_data_utility_matrix_weighted(partial_utility_matrix_df,preprocess_data_df):
    ...
    csv_path= os.path.join(DIR, '../data/pure_item_clustering/partial_users_items_utility_matrix.csv')
    relational_data_utility_matrix.to_csv(csv_path)
...
def create_utility_matrix_from_item_matrix(user_item_df,preprocess_data_df,partial_utility_matrix_df):
    ...
    path= os.path.join(DIR, '../data/pure_item_clustering/complete_utility_matrix.csv')
...
def preprocess_data(utility_matrix, n_items=100, n_users=100):
    ...
    kmeans = KMeans(n_clusters=200, random_state=0, n_init=10).fit(centered_matrix)
    ...
    csv_path= os.path.join(DIR, '../data/pure_item_clustering/centered_matrix.csv')
    centered_matrix.to_csv(csv_path)

    csv_path= os.path.join(DIR, '../data/pure_item_clustering/grouped_matrix.csv')
    grouped_matrix.to_csv(csv_path)

    for row in range(utility_matrix.shape[0]):
        for col in range(utility_matrix.shape[1]):
            cluster=int(centered_matrix.iloc[row,utility_matrix.shape[1]]) #movie cluster
            complete_utility_matrix.iloc[row,col]=utility_matrix.iloc[:, col].mean()+grouped_matrix.iloc[cluster,col]


    return utility_matrix_before_pp, partial_utility_matrix, centered_matrix,grouped_matrix,complete_utility_matrix
...
def print_data_to_html(utility_matrix_before_pp, partial_utility_matrix, centered_matrix, utility_matrix_complete, difference_utility_matrix):
    with open(os.path.join(DIR, '../data/pure_item_clustering/viz.html'), 'w') as f:
...
def plot_heatmap(utility_matrix, title, annot=True, n_rows=100, n_columns=100):
    ...
    # save the plot
    plt.savefig(os.path.join(DIR, '../data/pure_item_clustering/heatmap.png'))
    plt.show()
...
def save_top_k_items(partial_utility_matrix, utility_matrix_complete, top_k=10, n_users=10):
    path = '../data/pure_item_clustering/top_' + str(top_k) + '_items_n_' + str(n_users) + '_users.txt'
...
if __name__ == '__main__':
    ...
    utility_matrix_before_pp, new_partial_utility_matrix, centered_matrix,grouped_matrix,items_utility_matrix_complete = preprocess_data(new_partial_utility_matrix, n_items=N_ITEMS, n_users=N_USERS)
    ...
    path= os.path.join(DIR, '../data/pure_item_clustering/users_items_utility_matrix_complete.csv')
    ...
    print('--------------')
    log_to_txt(os.path.join(DIR, '../data/pure_item_clustering/performance.txt'), '--------------\n')
    print('Configuration: N_ITEMS =', N_ITEMS, ', N_USERS =', N_USERS)
    log_to_txt(os.path.join(DIR, '../data/pure_item_clustering/performance.txt'), 'Configuration: N_ITEMS = ' + str(N_ITEMS) + ', N_USERS = ' + str(N_USERS) + '\n')
    ...
    log_to_txt(os.path.join(DIR, '../data/pure_item_clustering/performance.txt'), 'Performance of the item-item collaborative filtering algorithm:\n')
    ...
    print('MAE: ', mae)
    log_to_txt(os.path.join(DIR, '../data/pure_item_clustering/performance.txt'), 'MAE: ' + str(mae) + '\n')
    ...
    print('RMSE :', rmse)
    log_to_txt(os.path.join(DIR, '../data/pure_item_clustering/performance.txt'), 'RMSE: ' + str(rmse) + '\n')

    mape = calculate_mape(real_utility_matrix_complete, utility_matrix_complete)
    print('MAPE :', mape)
    log_to_txt(os.path.join(DIR, '../data/pure_item_clustering/performance.txt'), 'MAPE: ' + str(mape) + '\n')

    mre = calculate_mre(real_utility_matrix_complete, utility_matrix_complete)
    print('MRE: ', mre)
    log_to_txt(os.path.join(DIR, '../data/pure_item_clustering/performance.txt'), 'MRE: ' + str(mre) + '\n')
