...
def calculate_rating(utility_matrix, centered_matrix, similarities, counter, user=4, query=0, top_n=2, verbose=False):
    ...
    for i in top_n_similarities:
        ...
        if np.isnan(utility_matrix.iloc[i, query]):
            rating += similarities[i] * utility_matrix.iloc[i, :].mean()
        else:
            rating += similarities[i] * utility_matrix.iloc[i, query]
    ...
    if similarities[top_n_similarities].sum() == 0:
        ...
        # option 2: return the mean of the query
...
def compute_difference(utility_matrix_complete, n_queries=100, n_users=100):
    ...
    real_utility_matrix_complete = real_utility_matrix_complete.iloc[:n_users, :n_queries]
...
def get_top_k_queries(partial_utility_matrix, utility_matrix_complete, user=0, top_k=10):
    ...
    query_rating_list = []
    for query in range(partial_utility_matrix.shape[1]):
        # if the query is not rated by the user
        if np.isnan(partial_utility_matrix.iloc[user, query]):
            query_rating_list.append((query, utility_matrix_complete.iloc[user, query]))
...
def save_top_k_queries(partial_utility_matrix, utility_matrix_complete, top_k=10, n_users=10):
    path = os.path.join(DIR,'../data/top_' + str(top_k) + '_queries_n_' + str(n_users) + '_users.txt')
...
if __name__ == '__main__':
    ...
    plot_heatmap(difference_utility_matrix, 'Difference', annot=False, n_rows=N_USERS, n_columns=N_QUERIES)

    real_utility_matrix_complete = import_data('real_complete')
    real_utility_matrix_complete = prepare_matrix(real_utility_matrix_complete, n_queries=N_QUERIES, n_users=N_USERS)

    print('--------------')
    log_to_txt(os.path.join(DIR, '../../data/compact_user_user_cf/performance.txt'), '--------------\n')
    print('Configuration: N_QUERIES =', N_QUERIES, ', N_USERS =', N_USERS)
    log_to_txt(os.path.join(DIR, '../../data/compact_user_user_cf/performance.txt'), 'Configuration: N_QUERIES = ' + str(N_QUERIES) + ', N_USERS = ' + str(N_USERS) + '\n')
    ...
    log_to_txt(os.path.join(DIR, '../../data/compact_user_user_cf/performance.txt'), 'Performance of the user-user collaborative filtering algorithm:\n')
    ...
    print('MAE: ', mae)
    log_to_txt(os.path.join(DIR, '../../data/compact_user_user_cf/performance.txt'), 'MAE: ' + str(mae) + '\n')
    ...
    print('RMSE :', rmse)
    log_to_txt(os.path.join(DIR, '../../data/compact_user_user_cf/performance.txt'), 'RMSE: ' + str(rmse) + '\n')

    mape = calculate_mape(real_utility_matrix_complete, utility_matrix_complete)
    print('MAPE :', mape)
    log_to_txt(os.path.join(DIR, '../../data/compact_user_user_cf/performance.txt'), 'MAPE: ' + str(mape) + '\n')

    mre = calculate_mre(real_utility_matrix_complete, utility_matrix_complete)
    print('MRE: ', mre)
    log_to_txt(os.path.join(DIR, '../../data/compact_user_user_cf/performance.txt'), 'MRE: ' + str(mre) + '\n')
